...
#memoryLeak_phrase = '     Please report this message to LST' # Linux
memoryLeak_phrase = 'ntdll.dll          00007FFA0FFC26B1  Unknown               Unknown  Unknown' # Windows
    ...
    for i_restarts in range(0,n_restarts):
        ...
        if ( ( i_restarts>=1 or force_to_start_with_fresh_run==False ) and len(result)>0 ):
            if (os.path.exists(exeName)):
                ...
                proc = subprocess.Popen([exeName," r=runrsf ncpu=8 memory=20m"], cwd=folder, stdout=subprocess.PIPE, stderr=subprocess.STDOUT) # Windows
        ...
        # If it is the first try or there is no runrsf-file in the later try, use the standard "run"
            ...
            # proc = subprocess.Popen(["sh","./0_run.asc"], cwd=folder, stdout=subprocess.PIPE, stderr=subprocess.STDOUT) # Linux
            proc = subprocess.Popen([exeName," i="+"1_main.k ncpu=8 memory=20m"], cwd=folder, stdout=subprocess.PIPE, stderr=subprocess.STDOUT) # Windows
            output_screen_file(proc, folder, i_restarts)
